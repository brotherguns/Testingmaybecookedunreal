...
void Off::InSDK::ProcessEvent::InitPE_Apple()
{
	...
	/*
	 * ARM64 ProcessEvent check:
	 * ProcessEvent always tests UFunction::FunctionFlags with masks 0x04000000 (FUNC_Native)
	 * and 0x00400000 (FUNC_BlueprintCallable). On ARM64, this manifests as a TBNZ or TST
	 * instruction against a word-offset into the UFunction. We approximate the Windows
	 * check by looking for any function that contains two TST/TBNZ instructions referencing
	 * the known FunctionFlags offset.
	 */
	auto IsProcessEvent = [](const uint8_t* FuncAddress, [[maybe_unused]] int32_t Index) -> bool
	{
		constexpr int32 ScanRange = 0xF00;
		...
		for (int i = 0; i < ScanRange; i += 4)
		{
			...
			// TST (immediate) on ARM64: top bits 1111 0010 00 xxxxxxxx
			// TBNZ: top bits 0111 0111 xxxxxxxx
			// We broadly look for TST/TBNZ patterns that access a word near Off::UFunction::FunctionFlags offset
			const bool IsTST  = (Instr & 0xFF000000) == 0x72000000; // TST (imm, 32-bit)
			const bool IsTBNZ = (Instr & 0x7F000000) == 0x37000000; // TBNZ
			const bool isTSTReg = (Instr & 0xFF200C00) == 0x6A000000; // TST (shifted reg)

			if (IsTST || IsTBNZ || isTSTReg)
				MatchCount++;
	...
	if (!FuncPtr)
	{
		/* Fallback: "Accessed None" string is near ProcessEvent on ARM64 too */
		const void* StringRefAddr = Platform::FindByStringInAllSections(L"Accessed None", 0x0, 0x0, Settings::General::bSearchOnlyExecutableSectionsForStrings);
		if (StringRefAddr)
		{
			/* ProcessEvent is typically the next function after the one containing this string */
			const uintptr_t Base = reinterpret_cast<uintptr_t>(StringRefAddr);
			/* Try to find the next function boundary by scanning for a prologue in the 0x400 bytes after */
			uintptr_t NextFuncAddr = 0;
			for (int i = 4; i < 0x400 && !NextFuncAddr; i += 4)
			{
				const uint32_t Instr = *reinterpret_cast<const uint32_t*>(Base + i);
				/* STP x29, x30, [sp, ...] is 0xA9BF7BFD */
			...
			if (NextFuncAddr)
			{
				auto IsSameAddr = [NextFuncAddr](const uint8_t* FuncAddress, [[maybe_unused]] int32_t Index) -> bool
				{
					return reinterpret_cast<uintptr_t>(FuncAddress) == NextFuncAddr;
				};
