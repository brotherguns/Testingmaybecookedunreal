...
// ============================================================================
//  Apple / ARM64 implementation
// ============================================================================
void FName::Init_Apple(bool bForceGNames)
{
#ifdef PLATFORM_APPLE

	/*
	 * On ARM64, FName::AppendString is typically found by:
	 *  1. Locate the string "ForwardShadingQuality_" via ADRP+ADD scanning.
	 *  2. From that instruction, look for a BL nearby that reaches AppendString.
	 *
	 * ARM64 BL is encoded as:  1001 01 imm26
	 * We check for that and resolve the target.
	 */

	const void* StringRef = Platform::FindByStringInAllSections("ForwardShadingQuality_", 0x0, 0x0, Settings::General::bSearchOnlyExecutableSectionsForStrings);

	if (StringRef && !bForceGNames)
	{
		/*
		 * Walk forward up to 0x80 bytes from the ADRP instruction that
		 * references "ForwardShadingQuality_", looking for a BL instruction.
		 * The first BL we hit that resolves to an address inside the image
		 * is almost certainly AppendString.
		 */
		const uintptr_t Base = reinterpret_cast<uintptr_t>(StringRef);

		for (int i = 0; i < 0x80; i += 4)
		{
			const uintptr_t InstrAddr = Base + i;
			if (Architecture_ARM64::IsBL(InstrAddr))
			{
				const uintptr_t Target = Architecture_ARM64::ResolveBL(InstrAddr);
				if (Platform::IsAddressInProcessRange(Target))
				{
					AppendString = reinterpret_cast<decltype(AppendString)>(Target);
					break;
				}
			}
		}
		...
		/*
		 * If not found forward, search backward a little (caller may be above).
		 */
		if (!AppendString)
		{
	...
	if (!AppendString)
		AppendString = static_cast<decltype(AppendString)>(TryFindApendStringBackupStringRef_Apple());
...
void* FName::TryFindApendStringBackupStringRef_Apple()
{
#ifdef PLATFORM_APPLE

	/*
	 * On ARM64 we try the " Bone: " string as a backup.
	 * Look for a BL in the vicinity of the ADRP that loads " Bone: ".
	 */
	const void* StringRef = Platform::FindByStringInAllSections(L" Bone: ", 0x0, 0x0, Settings::General::bSearchOnlyExecutableSectionsForStrings);
	...
	if (StringRef)
	{
		...
		// Search up to 0xB0 bytes before the string reference
...
void FName::InitFallback()
{
	...
	/*
	 * ARM64 fallback: scan for BL instructions near Conv_NameToString.
	 * The first BL that resolves to an in-module address is FName::ToString.
	 */
	if (Conv_NameToStringAddress)
	{
		const uintptr_t Base = reinterpret_cast<uintptr_t>(Conv_NameToStringAddress);
		for (int i = 0; i < 0x90 && !AppendString; i += 4)
		{
			const uintptr_t InstrAddr = Base + i;
			if (Architecture_ARM64::IsBL(InstrAddr))
			{
