...
// ============================================================================
//  Apple / ARM64 implementations
// ============================================================================

/*
 * On ARM64, FNamePool::FNamePool is found by:
 *  1. ADRP+ADD pair that loads "ByteProperty" string → identifies the constructor
 *  2. Back-track from that ADRP to the function prologue
 *  3. Find an ADRP+ADD pair earlier in that function that loads the singleton address
 */
bool NameArray::TryFindNamePool_Apple()
{
#ifdef PLATFORM_APPLE

	constexpr int32 BytePropertySearchRange = 0x800;

	/* Find a reference to "ByteProperty" — this is inside FNamePool::FNamePool */
	const uint8* BytePropertyRef = static_cast<const uint8*>(
		Platform::FindByStringInAllSections("ByteProperty", 0x0, 0x0, Settings::General::bSearchOnlyExecutableSectionsForStrings));

	if (!BytePropertyRef)
	{
		/* Try wide version */
		BytePropertyRef = static_cast<const uint8*>(
			Platform::FindByStringInAllSections(L"ByteProperty", 0x0, 0x0, Settings::General::bSearchOnlyExecutableSectionsForStrings));
	}
	...
	/*
	 * Walk backward from the ByteProperty ADRP to find the function start
	 * (ARM64 function prologues start with STP x29, x30, [sp, #-N]! or SUB sp, sp, #N).
	 * We search backward up to 0x500 bytes.
	 */
	const uintptr_t BytePropertyInstr = reinterpret_cast<uintptr_t>(BytePropertyRef);
	uintptr_t FuncStart = 0;

	for (int i = 4; i < 0x500; i += 4)
	{
		const uintptr_t Addr = BytePropertyInstr - i;
		const uint32_t Instr = *reinterpret_cast<const uint32_t*>(Addr);

		/* STP x29, x30 = 0xA9BF7BFD pattern (common prologue) */
		if ((Instr & 0xFFC07FFF) == 0xA9007BFD || /* STP x29, x30, [sp, ...] */
		    (Instr & 0xFF800000) == 0xD1000000)     /* SUB sp, sp, #imm */
		{
			FuncStart = Addr;
			break;
		}
	...
	if (!FuncStart)
		FuncStart = BytePropertyInstr - 0x200; /* fallback: use some range before ByteProperty */

	/*
	 * Now search forward from the function start for an ADRP that resolves to
	 * a pointer-sized global (the FNamePool singleton).
	 * The singleton is passed as the first argument (x0) before calling the constructor.
	 * We look for ADRP+ADD pairs and check if the resolved address contains a valid pointer.
	 */
	for (int i = 0; i < 0x400 && (FuncStart + i) < BytePropertyInstr; i += 4)
	{
		const uintptr_t Addr = FuncStart + i;

		if (!Architecture_ARM64::IsADRP(Addr))
			continue;

		const uintptr_t Target = Architecture_ARM64::ResolveADRPPair(Addr);
		if (!Target || !Platform::IsAddressInProcessRange(Target))
			continue;

		if (Platform::IsBadReadPtr(reinterpret_cast<const void*>(Target)))
			continue;

		/* Check the resolved address holds a pointer to something readable (the pool itself) */
		const void* PossiblePool = *reinterpret_cast<const void* const*>(Target);
		if (!PossiblePool || Platform::IsBadReadPtr(PossiblePool))
			continue;

		Off::InSDK::NameArray::GNames = Platform::GetOffset(Target);
		return true;
	}
...
/*
 * TNameEntryArray (old GNames) on ARM64:
 * FName::GetNames() returns a TNameEntryArray*. We find it by locating a BL to a
 * function that returns a global, then dereferencing.
 */
bool NameArray::TryFindNameArray_Apple()
{
#ifdef PLATFORM_APPLE

	/*
	 * Find "ByteProperty" narrow string reference — it appears inside FName::StaticInit,
	 * which calls GetNames(). From the ADRP referencing it, walk backward to find a BL
	 * to GetNames, then dereference the returned pointer.
	 */
	const uint8* BytePropertyRef = static_cast<const uint8*>(
		Platform::FindByStringInAllSections("ByteProperty", 0x0, 0x0, Settings::General::bSearchOnlyExecutableSectionsForStrings));
	...
	const uintptr_t Base = reinterpret_cast<uintptr_t>(BytePropertyRef);

	/* Look for a BL instruction in the ~0x300 bytes before the "ByteProperty" reference */
	for (int i = 4; i < 0x300; i += 4)
	{
		const uintptr_t Addr = Base - i;
		if (!Architecture_ARM64::IsBL(Addr))
			continue;

		const uintptr_t GetNamesTarget = Architecture_ARM64::ResolveBL(Addr);
		if (!Platform::IsAddressInProcessRange(GetNamesTarget))
			continue;

		/*
		 * Call GetNames() to get the TNameEntryArray* pointer.
		 * Then search forward in GetNames() for an ADRP+LDR that loads GNames.
		 */
		using GetNamesFunc = void* (*)();
		void* Names = reinterpret_cast<GetNamesFunc>(GetNamesTarget)();

		if (!Names || Platform::IsBadReadPtr(Names))
			continue;

		/* Walk GetNames() looking for an ADRP that resolves to an address holding 'Names' */
		for (int j = 0; j < 0x80; j += 4)
		{
			const uintptr_t InstrAddr = GetNamesTarget + j;
			if (!Architecture_ARM64::IsADRP(InstrAddr))
				continue;

			const uintptr_t MoveTarget = Architecture_ARM64::ResolveADRPPair(InstrAddr);
			if (!MoveTarget || !Platform::IsAddressInProcessRange(MoveTarget))
				continue;

			if (Platform::IsBadReadPtr(reinterpret_cast<const void*>(MoveTarget)))
				continue;

			const void* Value = *reinterpret_cast<const void* const*>(MoveTarget);
			if (Value == Names)
			{
				Off::InSDK::NameArray::GNames = Platform::GetOffset(MoveTarget);
				return true;
